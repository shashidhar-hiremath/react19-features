...
const Feature: React.FC<FeatureProps> = ({ feature, isFullscreen }) => {
  ...
  return (
    <section className={`${!isFullscreen ? "bg-gradient-to-r from-white to-gray-50 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 mb-6 overflow-hidden" : ""}`}>
      <div className="p-6">
        {/* Header */}
                  ...
                  <ul className="grid gap-2">
                    {feature.features.map((feature, i) => (
                      ...
                      >
                        ...
                        <span>{feature.title ? <strong>{feature.title}: </strong> : null}{feature.description}</span>
                      </li>
                    ))}
                  </ul>
                )}
              ...
              {isBenefitsExpanded && (
                <ul className="grid gap-2">
                  {feature.benefits.map((benefit, i) => (
                    ...
                    >
                      <span className="text-blue-500 mt-1">•</span>
                      <span>{benefit.title ? <strong>{benefit.title}: </strong> : null}{benefit.description}</span>
                    </li>
                  ))}
                </ul>
              )}
          ...
          <div className="md:w-1/2 md:pl-4">
            <div className="relative h-full">
              ...
              <div className="absolute top-2 right-2">
                <a target="_blank" href={feature.codeUrl} className="bg-white/90 hover:bg-white px-3 py-1 rounded text-sm shadow-sm border border-gray-200">
                  View Full Code
                </a>
              </div>
            </div>
